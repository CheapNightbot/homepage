...
export default function Todo({
  ...
  function hideInput(e?: KeyboardEvent): void {
    if (!e) return;
    if (e.key === "Escape") {
      setInputVisible(!inputVisible);
      setTodoInput("");
  ...
  function handleInputBlur(e: FocusEvent<HTMLInputElement, Element>): void {
    e.target.removeEventListener("keydown", hideInput);
  }
    ...
    >
      <h2
        className={cn(
          ...
          inputVisible &&
            "animate-[exit_.15s_ease_0s_1_normal_forwards] blur-out",
        )}
      >
        Todo ☆⌒(ゝ。∂)
      </h2>
      <div className="overflow-clip mb-2">
        <form onSubmit={handleNewTodoSubmit}>
          <Input
            ...
            className={cn(
              "focus-visible:ring-0 transition-all duration-500 ease-in-out",
              inputVisible
                ? "animate-[enter_.15s_ease_0s_1_normal_forwards] slide-in-from-top"
      ...
      {todoList.length > 0 ? (
        <ScrollArea
          ...
          className="p-2 flex-1 max-h-[calc(100%-6rem)]"
        >
              ...
              return (
                <li
                  ...
                  className={cn(
                    ...
                    todo.status === "done" && editingId !== todo.id
                      ? "line-through text-primary"
                      : "",
                    hoverId === todo.id && "text-destructive bg-accent/50",
                    deletingId === todo.id &&
                      "animate-out slide-out-to-left ease-out duration-300",
                  )}
                >
                  {todo.status === "done" ? (
                    <SquareCheckBig
        ...
        <div className="animate-in fade-in duration-300 ease-in-out gap-2 p-2 flex-1 max-h-[calc(100%-8rem)] flex flex-col items-center justify-center">
          ...
          <p className="inline-flex items-center gap-1.5">
            ...
            <Plus
              size={18}
              className="bg-primary text-primary-foreground shadow-xs rounded-full p-px"
            />
      ...
      >
        <PlusIcon
          className={cn(
            ...
            inputVisible ? "rotate-45" : "rotate-0",
          )}
        />
      </Button>
      <div className="flex items-center justify-evenly gap-2 text-sm fixed w-full left-0 bottom-0 py-2 bg-accent/20 backdrop-blur z-1 px-10">
        ...
        <Separator orientation="vertical" className="bg-foreground/80" />
        <p>
          Completed: {todoList.filter((todo) => todo.status === "done").length}
        </p>
      </div>
    </Window>
  );
